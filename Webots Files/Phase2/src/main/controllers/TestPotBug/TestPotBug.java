...
public class TestPotBug{
  ...
  private static final double DEFAULT_ROBOT_RADIUS = 1.0; //meters
  ...
  //private static final double DEFAULT_APROACH_STEP_SIZE = 2;
  private static final double DEFAULT_ENGAGE_STEP_SIZE = 1;
  ...
  private static final double DEFAULT_MAX_SAMPLE_VAL = 28; // meters.

  //Default goal positions
  private static final double DEFAULT_GOAL_POSITION_X = 15.0;
  private static final double DEFAULT_GOAL_POSITION_Y = 3.0;
  ...
  public TestPotBug(GPS gps, Compass compass, Emitter emitter, Motor frontLeft,
  ...
  public static void main(String[] args){
    ...
    while (robot.step(timeStep*100) != -1) {
      ...
      if(!potBugController.isGoalReached() && potBugController.isRunning){
        //Check for goal direction on forwards semi-circle
        if (!potBugController.isObstruction()){

          potBugController.setDriveMode(DEFAULT_DRIVE_MODE_TOWARDS_GOAL);
          //Move 'freely' to the next sample point, towards goal
          potBugController.updateMetrics();

        } else {

          if (potBugController.isClearToLeave()){

            if (potBugController.isProgressAttained() &&
                potBugController.isClearToLeave()){
            ...
            if (potBugController.hitPoint == null){

              potBugController.setHitPoint();
              potBugController.setDriveMode(DEFAULT_DRIVE_MODE_TOWARDS_CONTOUR);
              potBugController.updateMetrics();

            } else {

              // engaged mode
              potBugController.setDriveMode(DEFAULT_DRIVE_MODE_ALONG_CONTOUR);
              potBugController.updateMetrics();
  ...
  private boolean isGoalReached(){
    ...
    System.out.println("Current Position :: "+this.goalNode.getPosition());
